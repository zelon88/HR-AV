...
Function createEmail(emailSubject, emailContent)
  ...
  oFile.Write "To: " & Sanitize(toEmail) & vbNewLine & "From: " & Sanitize(strComputerName) & "@" & Sanitize(companyDomain) & vbNewLine & _
   ...
   vbNewLine & "USER NAME: " & Sanitize(strCrurentUserName) & vbNewLine & "WORKSTATION: " & Sanitize(strComputerName) & vbNewLine & _
...
'--------------------------------------------------
'A function to create a startup entry in C:\Users\USER\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup.
'https://www.vistax64.com/threads/vbs-script-to-create-start-programs-menu-item.240145/
Function createStartupEntry()
  Set oLNK = objShell.CreateShortcut("C:\Users\" & strCurrentUserName & "\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\" & appname & ".lnk")
  With oLNK
...
'--------------------------------------------------
'A function to create a startup entry in "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\" appName ".lnk".
'https://www.vistax64.com/threads/vbs-script-to-create-start-programs-menu-item.240145/
Function createStartMenuShortcut()
  ...
  End With
  oLNK = NULL
  Set oLNK = objShell.CreateShortcut("C:\Users\Public\Desktop\" & appname & ".lnk")
  With oLNK
  .TargetPath = InstallationDirectory & appname & "\" & appName & ".hta"
  .IconLocation = InstallationDirectory & appName & "\Resources\" & appName & ".ico"
  .Save
  End With
  oLNK = NULL
  Set oLNK = objShell.CreateShortcut("C:\Users\" & strCurrentUserName & "\Desktop\" & appname & ".lnk")
  With oLNK
  .TargetPath = InstallationDirectory & appname & "\" & appName & ".hta"
  .IconLocation = InstallationDirectory & appName & "\Resources\" & appName & ".ico"
  .Save
...
Function installationWizard1()
  ...
  instMsg7 = "Would you like " & appName & " to start when Windows starts?"
  instMsg8 = "Your new software is ready for use."
  ...
  If dontContinue = FALSE And result0 <> 2 And result0 <> 3 And pfCopyResult = TRUE And objFSO.FileExists(InstallationDirectory & appName & "\" & appName & ".hta") Then
    ...
    result1 = Msgbox(instMsg7, vbYesNo, instHead)
    If result1 = vbYes Then
      createStartupEntry()
    End If
    PrintGracefully InstHead, instMsg8, "vbOkOnly"
    'objShell.Run """C:\Program Files\HR-AV\Scripts\VBS\Restart.vbs"""
    DieGracefully 0, instMsg8, TRUE 
  Else
...
'If these are ever converted into .exe files each binary will have to be manually specified here.
'Leaves HTA's running.
Function killAllScripts()
  killAllScripts = TRUE
  createLog("Attempting to kill running scripts.")
  objShell.Run "C:\Windows\System32\cmd.exe /c taskkill /im wscript.exe /fi ""USERNAME eq %username%""", 0, FALSE
  objShell.Run "C:\Windows\System32\cmd.exe /c taskkill /im cscript.exe /fi ""USERNAME eq %username%""", 0, FALSE 
...
'--------------------------------------------------
'A function to kill all HTA's. 
'Leaves wscript, cscript, & cmd running.
'If these are ever converted into .exe files each binary will have to be manually specified here.
Function killAllHTAS()
  killAllHTAS = TRUE
  createLog("Attempting to kill running HTA's.")
  objShell.Run "C:\Windows\System32\cmd.exe /c taskkill /im mshta.exe /fi ""USERNAME eq %username%""", 0, FALSE 
  searchScripts = enumerateRunningProcesses()
  For Each scriptsToSearch In searchScripts
    If InStr(LCase(scriptsToSearch), "mshta") > 0 Then
      killAllHTAS = FALSE
    End If
  Next
End Function 
